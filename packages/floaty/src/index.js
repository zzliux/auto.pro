...
import { exhaustMap, filter, map, share, shareReplay, skipUntil, startWith, switchMap, take, takeUntil, tap, withLatestFrom } from 'rxjs/operators';
var icons = [
...
export function createFloaty(_a) {
    ...
    function animation() {
        return new Promise(function (resolve, reject) {
            ...
            var direction = STAND.getX() < getWidthPixels() / 2 ? 1 : -1;
            var base = Math.floor(angle / (items.length - 1));
            var r = Math.floor(radius * DPI - (SIZE_PIXELS / 2 + 2));
            var animationItems = [];
            var α = angle / 2;
    ...
    function checkFloatyPosition() {
        var logoX = FLOATY.getX() + FLOATY_STAND_OFFSET_X;
    ...
    down$.pipe(map(function (e) { return ({ dx: e.getRawX(), dy: e.getRawY(), sx: STAND.getX(), sy: STAND.getY() }); }), switchMap(function (_a) {
        ...
        return merge(
        // 按下后无移动，则弹起时视为点击
        up$.pipe(takeUntil(move$), tap(toggleFloaty)), move$.pipe(tap(function (e_move) {
